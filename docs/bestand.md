...
|---|---------|--------|------|-----|--------|-------|-------------|
| 1 | **PATCH** | ✅ | ✅ | ✅ | ✅ | ✅ | JSON Patch (`application/json-patch+json`) + FHIRPath Patch (`Parameters`). Partiële updates zijn een core FHIR REST interactie. |
| 2 | **Compartment Search** | ✅ | ✅ | ✅ | ✅ | ✅ | `GET /Patient/123/Observation` — scoped search binnen patient/encounter/practitioner compartiment. Fundamenteel FHIR concept. |
| 3 | **Terminology Services** | ✅ | ✅ | ✅ | ✅ | — | `$expand` (ValueSet), `$lookup` (CodeSystem), `$validate-code`, `$subsumes`. Kritiek voor validatie en klinische workflows. |
...
| 10 | **CORS** | ✅ | ✅ | ✅ | ✅ | ✅ | Dedicated CORS configuratie voor browser-based SMART apps. Preflight OPTIONS, allowed origins/headers. |
...
| 15 | **$translate** | ✅ | ✅ | ✅ | — | — | Code vertaling via ConceptMap resources (bijv. SNOMED → ICD-10). |
...
### Fase 2 — Klinische waarde
4. **Terminology Services** ($expand, $lookup, $validate-code) — kritiek voor validatie workflows
...
### Fase 4 — Geavanceerd
11. **$expunge** (GDPR)
12. **$translate** (terminology)
13. **GraphQL**
14. **Multi-tenancy**
15. **Consent-based Access Control**