...
export class SearchParameterRegistry implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    try {
      ...
      for (const r of docs) {
        ...
        const def: SearchParamDef = {code: r.code, type: r.type as SearchParamType, expression: r.expression, base: r.base || [],
  ...
  /**
   * Reloads all search parameter definitions from MongoDB.
   * Clears existing caches and re-populates the maps.
   * Call this after creating/updating/deleting SearchParameter resources.
   * @returns The number of parameters loaded.
   */
  async reload(): Promise<number> {
    this.paramMap.clear();
    this.pathCache.clear();
    this.paramsByType.clear();
  ...
  getParam(resourceType: string, code: string): SearchParamDef | undefined {
    ...
    return this.paramMap.get(`${resourceType}:${code}`) || this.paramMap.get(`Resource:${code}`) ||
      this.paramMap.get(`DomainResource:${code}`);
  }
