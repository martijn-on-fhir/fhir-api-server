  ...
  map(stu3: any): MapperResult {
    ...
    for (const role of roles) {
      try {
        ...
        if (role.code) {
          prRole.code = Array.isArray(role.code) ? role.code : [role.code];
        }

        if (role.specialty) {
          prRole.specialty = Array.isArray(role.specialty) ? role.specialty : [role.specialty];
        }

        if (role.organization) {
          prRole.organization = role.organization;
        }

        if (role.location) {
          prRole.location = Array.isArray(role.location) ? role.location : [role.location];
        }

        if (role.identifier) {
          prRole.identifier = Array.isArray(role.identifier) ? role.identifier : [role.identifier];
        }

        if (role.period) {
          prRole.period = role.period;
        }
